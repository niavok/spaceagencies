...
public class ContinueGameActivity extends Activity {
    ...
    public void onCreate(Bundle bundle) {
        ...
        if(GameClient.getInstance().getGameManager().getActiveGame() != null) {
            // Game already started, just resume the game.
            getHandler().obtainMessage(NEW_GAME_CREATED_WHAT).send();
        } else {
        
            MetaGame game = GameClient.getInstance().getGameManager().getPreviousGame();
